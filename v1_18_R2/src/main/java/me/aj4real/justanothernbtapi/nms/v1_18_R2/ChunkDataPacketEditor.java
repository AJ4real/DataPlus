...
package me.aj4real.justanothernbtapi.nms.v1_18_R2;
...
public class ChunkDataPacketEditor {

    public static final Field chunkdataBuffer;
    public static final Field packetChunkData;
    public static final Constructor<?> palettedContainer;

    static {
        Class<ClientboundLevelChunkPacketData> chunkpacket = ClientboundLevelChunkPacketData.class;
        Class<ClientboundLevelChunkWithLightPacket> chunkWithLightPacket = ClientboundLevelChunkWithLightPacket.class;
        chunkdataBuffer = Arrays.stream(chunkpacket.getDeclaredFields()).filter((f) -> f.getType() == byte[].class).findFirst().get();
        packetChunkData = Arrays.stream(chunkWithLightPacket.getDeclaredFields()).filter((f) -> f.getType() == ClientboundLevelChunkPacketData.class).findFirst().get();
        palettedContainer = Arrays.stream(PalettedContainer.class.getConstructors()).filter((c) -> c.getParameterCount() == 3).findAny().get();
    }
    ...
    public ChunkDataPacketEditor(Chunk chunk) throws InvocationTargetException, InstantiationException, IllegalAccessException {
    ...
    public ChunkDataPacketEditor(World world, ClientboundLevelChunkWithLightPacket packet) throws InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        for (int y = 0; y < size; y++) {
            ...
            PalettedContainer<BlockState> states = (PalettedContainer<BlockState>) palettedContainer.newInstance(Block.BLOCK_STATE_REGISTRY, Blocks.AIR.defaultBlockState(), PalettedContainer.Strategy.SECTION_STATES);
            states.read(tmp);
            this.states[y] = states;
            PalettedContainer<net.minecraft.world.level.biome.Biome> biomes = (PalettedContainer<net.minecraft.world.level.biome.Biome>) palettedContainer.newInstance(registry, registry.getOrThrow(Biomes.THE_VOID), PalettedContainer.Strategy.SECTION_BIOMES);
            biomes.read(tmp);
    ...
    public void setAllNMSBiome(net.minecraft.world.level.biome.Biome biome) {
        for (int i = 0; i < this.biomes.length; i++) {
            PalettedContainer<net.minecraft.world.level.biome.Biome> palette = this.biomes[i];
            palette.acquire();
            for (int x = 0; x < 4; x++) {
    ...
    public boolean setNMSBiome(int x, int y, int z, net.minecraft.world.level.biome.Biome biome) {
        PalettedContainer<net.minecraft.world.level.biome.Biome> palette = this.biomes[this.getSectionIndex(y)];
        palette.acquire();
        palette.set((x & 15) >> 2, (y & 15) >> 2, (z & 15) >> 2, biome);
        palette.release();
